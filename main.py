...
ELEVEN_VOICE_ID = os.getenv("ELEVEN_VOICE_ID", "2EiwWnXFnvU5JabPnv8n") 
...
def call_gemini(prompt: str, temperature: float = 0.2, max_tokens: int = 300):
    """Call Gemini 2.5 Pro safely and handle empty 'Part' issues."""
    try:
        ...
        try:
            ...
            print("Gemini parse warning:", parse_err)

        if not text:
            print("Retrying Gemini with simplified prompt...")
            fallback_prompt = (
                "Respond briefly and politely to the following user message:\n"
                + prompt[-1200:]
            )
            ...
            try:
                ...
                print("Gemini parse retry warning:", parse_err)

        if not text:
            text = "Namaste! Main theek hoon — batayein, aapko kis cheez mein madad chahiye?"
